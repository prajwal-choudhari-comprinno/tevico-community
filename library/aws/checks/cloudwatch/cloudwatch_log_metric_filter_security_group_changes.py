...
class cloudwatch_log_metric_filter_security_group_changes(Check):
    ...
    def execute(self, connection: boto3.Session) -> CheckReport:
        ...
        try:
            # Define the security group changes pattern
            pattern = re.compile(r"\$\.eventName\s*=\s*.?AuthorizeSecurityGroupIngress.+\$\.eventName\s*=\s*.?AuthorizeSecurityGroupEgress.+\$\.eventName\s*=\s*.?RevokeSecurityGroupIngress.+\$\.eventName\s*=\s*.?RevokeSecurityGroupEgress.+\$\.eventName\s*=\s*.?CreateSecurityGroup.+\$\.eventName\s*=\s*.?DeleteSecurityGroup.?")
            ...
            # Filter metric filters that match the security group changes pattern
            ...
            if not filtered_metric_filters:
                report.resource_ids_status.append(
                    ResourceStatus(
                        ...
                        summary="No metric filters found matching the security group changes pattern."
                    )
                )
            ...
            # Iterate over log groups with security group changes metric filters
                ...
                if alarm_names:
                    report.resource_ids_status.append(
                        ResourceStatus(
                            ...
                            summary=f"Log group {log_group_name} has metric filter '{filter_name}' for security group changes and associated alarms: {', '.join(alarm_names)}."
                        )
                    )
                else:
                    report.resource_ids_status.append(
                        ResourceStatus(
                            resource=AwsResource(arn=log_group_arn),
                            status=CheckStatus.FAILED,
                            summary=f"Log group {log_group_name} has metric filter '{filter_name}' for security group changes but no associated alarms."
                        )
                    )
