...
class cloudwatch_log_metric_filter_unauthorized_api_calls(Check):
    def execute(self, connection: boto3.Session) -> CheckReport:
        ...
        try:
            ...
            # Define the custom pattern for unauthorized API calls
                    ...
                    if matching_filters:
                        report.resource_ids_status.append(
                            ResourceStatus(
                                ...
                                summary=f"Log group {log_group_name} has unauthorized API call metric filters configured: {', '.join(matching_filters)}."
                            )
                        )
                    else:
                        ...
                        report.resource_ids_status.append(
                            ResourceStatus(
                                ...
                                summary=f"Log group {log_group_name} does not have unauthorized API call metric filters configured."
                            )
                        )
