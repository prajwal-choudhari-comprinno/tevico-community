...
class cloudwatch_log_metric_filter_unauthorized_api_calls(Check):
    ...
    def execute(self, connection: boto3.Session) -> CheckReport:
        ...
        try:
            # Define the unauthorized API calls pattern
            pattern = re.compile(r"\$\.errorCode\s*=\s*.?\*UnauthorizedOperation.+\$\.errorCode\s*=\s*.?AccessDenied\*.?")
            ...
            # Filter metric filters that match the unauthorized API calls pattern
            ...
            if not filtered_metric_filters:
                report.resource_ids_status.append(
                    ResourceStatus(
                        ...
                        summary="No metric filters found matching the unauthorized API calls pattern."
                    )
                )
            ...
            # Iterate over log groups with unauthorized API calls metric filters
                ...
                if alarm_names:
                    report.resource_ids_status.append(
                        ResourceStatus(
                            ...
                            summary=f"Log group {log_group_name} has metric filter '{filter_name}' for unauthorized API calls and associated alarms: {', '.join(alarm_names)}."
                        )
                    )
                else:
                    report.resource_ids_status.append(
                        ResourceStatus(
                            resource=AwsResource(arn=log_group_arn),
                            status=CheckStatus.FAILED,
                            summary=f"Log group {log_group_name} has metric filter '{filter_name}' for unauthorized API calls but no associated alarms."
                        )
                    )
