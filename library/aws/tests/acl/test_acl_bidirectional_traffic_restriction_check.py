...
from library.aws.checks.acl.acl_bidirectional_traffic_restriction_check import acl_bidirectional_traffic_restriction_check
...
class TestAclBidirectionalTrafficRestrictionCheck:
    ...
    def setup_method(self):
        ...
        self.mock_session.client.side_effect = lambda service: self.mock_ec2 if service == "ec2" else self.mock_sts

    @patch("boto3.Session.client")
    def test_no_network_acls(self, mock_client):
    ...
    @patch("boto3.Session.client")
    def test_default_deny_rules(self, mock_client):
    ...
    @patch("boto3.Session.client")
    def test_permissive_ingress_egress(self, mock_client):
    ...
    @patch("boto3.Session.client")
    def test_permissive_ingress_only(self, mock_client):
    ...
    @patch("boto3.Session.client")
    def test_permissive_egress_only(self, mock_client):
    ...
    @patch("boto3.Session.client")
    def test_no_permissive_rules(self, mock_client):
    ...
    def test_client_error(self, mock_client):
        ...
        self.mock_ec2.describe_network_acls.side_effect = ClientError({"Error": {"Code": "AccessDenied"}}, "DescribeNetworkAcls")
        report = self.check.execute(self.mock_session)
        assert report.status == CheckStatus.FAILED
        assert report.resource_ids_status[0].summary is not None
